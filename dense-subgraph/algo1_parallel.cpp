...
class FlowNetwork {
  public:
    ...
    FlowNetwork(int n) : N{n}, graph{static_cast<size_t>(n)} {}
...
struct TriangleTask {
    ...
    vector<array<int, 3>> local_triangles;
};

void* triangleWorker(void* arg) {
    TriangleTask* task = (TriangleTask*) arg;
    const auto& adj = *(task->adj);

    for (int u = task->start_u; u < task->end_u; u++) {
            ...
            if (v > u) {
                ...
                while (i < su && j < sv) {
                    if (adj[u][i] == adj[v][j]) {
                        int w = adj[u][i];
                        if (w > v) {
                            task->local_triangles.push_back({u, v, w});
                        }
                        i++;
                        j++;
...
vector<array<int, 3>> getTriangles(int n, const vector<vector<int>>& adj) {
    ...
    vector<TriangleTask> tasks(num_threads);

    int chunk = (n + num_threads - 1) / num_threads;

    for (int t = 0; t < num_threads; t++) {
        tasks[t].start_u = t * chunk;
        tasks[t].end_u = min(n, (t + 1) * chunk);
        tasks[t].n = n;
        tasks[t].adj = &adj;

        pthread_create(&threads[t], nullptr, triangleWorker, &tasks[t]);
    }
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        triangles.insert(triangles.end(),
                         tasks[t].local_triangles.begin(),
                         tasks[t].local_triangles.end());
    }
...
void display_memory_usage(ostream& out) {
    ...
    // out << "Peak memory usage: " << usage.ru_maxrss / (1024 * 1024) << "MB" << endl;
    out << "Peak memory usage: " << usage.ru_maxrss / 1024 << "MB" << endl;
}
