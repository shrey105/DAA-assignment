    ...
    for (int u = 0; u < n; u++) {
        for (int v : adj[u]) if (v > u) {
            ...
            while (i < su && j < sv) {
                if (adj[u][i] == adj[v][j]) {
                    int w = adj[u][i];
                    if (w > v) triangles.push_back({u, v, w});
                    i++; j++;
...
vector<int> cliqueCoreDecomposition(int n,
    ...
    int max_deg = 0;
    for (int v = 0; v < n; v++) max_deg = max(max_deg, deg[v]);
    ...
    auto moveBin = [&](int v, int old_d, int new_d) {
        ...
        pos[last] = p;
        bin[old_d].pop_back();
        if ((int)bin.size() <= new_d) bin.resize(new_d + 1);
    ...
    int scan = 0;
    while (processed < n) {
        while (scan < (int)bin.size() && bin[scan].empty()) scan++;
        if (scan >= (int)bin.size()) break;
        ...
        if (remaining_v > 0) {
            ...
            if (d > best_residual_density) best_residual_density = d;
        }
    }
...
vector<vector<int>> connectedComponents(const vector<vector<int>>& adj,
    ...
    for (int v = 0; v < n; v++) {
        if (!in_sub[v] || visited[v]) continue;
        vector<int> comp;
        queue<int> q;
...
int countTrianglesInMask(const vector<bool>& mask,
    ...
    for (auto& t : triangles)
        if (mask[t[0]] && mask[t[1]] && mask[t[2]]) cnt++;
    return cnt;
}
    ...
    int& s_out, int& t_out)
{
    ...
    for (int i = 0; i < nc; i++) fn.addEdge(s, i, tri_deg[i]);
    // v -> t with cap = |V_Psi| * alpha = 3 * alpha
    for (int i = 0; i < nc; i++) fn.addEdge(i, t, 3.0 * alpha);
...
void refreshG2L(const vector<int>& comp, vector<int>& g2l) {
    ...
    for (int i = 0; i < (int)comp.size(); i++) g2l[comp[i]] = i;
}
...
vector<int> findDensestSubgraphCoreExact(const vector<vector<int>>& adj) {
    ...
    int kmax = 0;
    for (int v = 0; v < n; v++) kmax = max(kmax, core[v]);

    if (kmax == 0) return {};   // no triangles at all
    ...
    for (int v = 0; v < n; v++) if (core[v] >= k_prime) in_kprime[v] = true;
    ...
    for (auto& comp : comps_kprime) {
        ...
        for (int v : comp) mask[v] = true;
        double d = triangleDensityOfMask(mask, triangles, comp.size());
        if (d > rho_dprime) rho_dprime = d;
        if (d > seed_density) { seed_density = d; seed_D = comp; }
    }
    ...
    for (int v = 0; v < n; v++) if (core[v] >= k_dprime) in_kdprime[v] = true;
    ...
    for (auto comp : C) {                       // copy: we will shrink it
        ...
        if (l > (double)k_dprime + 1e-12) {
            int kc = ceilEps(l);
            vector<int> filt;
            for (int v : comp) if (core[v] >= kc) filt.push_back(v);
            comp = std::move(filt);
        }
        ...
        for (int i = 0; i < (int)comp.size(); i++)
            if (reach0[i]) { nonempty = true; break; }
        if (!nonempty) continue;            // skip this component

        // Initial best subgraph from this component = the feasible side at l
        vector<int> U;
        for (int i = 0; i < (int)comp.size(); i++)
            if (reach0[i]) U.push_back(comp[i]);
        ...
        while (u - l >= 1.0 / ((double)Vc * (Vc - 1) + 1e-12)) {
            ...
            vector<int> U_new;
            for (int i = 0; i < (int)comp.size(); i++) {
                if (reachA[i]) { any = true; U_new.push_back(comp[i]); }
            }
